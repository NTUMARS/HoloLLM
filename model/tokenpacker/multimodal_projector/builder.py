...
class TokenPacker(nn.Module):
    ...
    def divide_feature(self, x, kernel_size, token_num, N, c):
        ...
        # hw = int(token_num ** 0.5) 
        x = x.permute(1, 0, 2)
        hw = math.ceil(token_num ** 0.5) 
        if hw % 2 != 0:
            hw = hw + 1
        if x.size(1) != hw**2:  
            padding_size = hw**2 - x.size(1)
        ...
        reshape_x = x_padded.reshape(h, w, N, c).reshape(h//kernel_size, kernel_size, w, N, c)
        ...
        reshape_x = reshape_x.permute(2,0,1,3,4).reshape(kernel_size*kernel_size, -1, N, c)
    ...
    def forward(self, x, attn_mask=None):
        ...
        q = F.interpolate(x_padded.reshape(x.shape[0], hw, hw,-1).float().permute(0,3,1,2), size=(self.grid_size, self.grid_size), mode='bilinear').permute(0,2,3,1) ## fix

        # q = q.reshape(q.shape[0], -1, q.shape[-1]).to(x.dtype)
        if not self.training:
            q = q.reshape(q.shape[0], -1, q.shape[-1]).to(torch.half) 
        else:
            q = q.reshape(q.shape[0], -1, q.shape[-1]).to(torch.bfloat16)  
        ...
        if not self.training:
            reshape_query, reshape_key, reshape_value = reshape_query.squeeze().to(torch.half), reshape_key.reshape(-1, N, c).to(torch.half), reshape_value.reshape(-1, N, c).to(torch.half)
        else:
            reshape_query, reshape_key, reshape_value = reshape_query.squeeze().to(torch.bfloat16), reshape_key.reshape(-1, N, c).to(torch.bfloat16), reshape_value.reshape(-1, N, c).to(torch.bfloat16)
        out = self.clip_attn(
            reshape_query,
            reshape_key,
            reshape_value,
