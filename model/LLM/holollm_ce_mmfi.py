...
class Transformer(nn.Module):
        ...
        for modal in self.modals:
            ...
            if modal in ['video','depth', 'infra']:
                ...
                if modal == "video":
                    ...
                    pretrained_path = "./modality_specific_encoders/mmfi_encoders/ce_pretrained_encoders/rgb_network.pth"
                elif modal == "depth":
                    # self.convnet_conv_depth = nn.Conv1d(in_channels=256, out_channels=64, kernel_size=1, stride=1)
                    pretrained_path = "./modality_specific_encoders/mmfi_encoders/ce_pretrained_encoders/depth_network.pth"
                else:
                    pretrained_path = "./modality_specific_encoders/xrf55_encoders/ce_pretrained_encoders/infra_network.pth"
                ...
                pretrained_path = "./modality_specific_encoders/mmfi_encoders/ce_pretrained_encoders/lidar_network.pth"
                ...
                pretrained_path = "./modality_specific_encoders/mmfi_encoders/ce_pretrained_encoders/mmwave_network.pth"
                ...
                pretrained_path = "./modality_specific_encoders/mmfi_encoders/ce_pretrained_encoders/wifi_network.pth"
                checkpoint = torch.load(pretrained_path)
                encoder_state_dict = self.modality_encoders[modal].state_dict()
                for k, v in encoder_state_dict.items():
                ...
                pretrained_path = "./modality_specific_encoders/xrf55_encoders/ce_pretrained_encoders/rfid_network.pth"
                checkpoint = torch.load(pretrained_path)
                encoder_state_dict = self.modality_encoders[modal].state_dict()
                for k, v in encoder_state_dict.items():
                    encoder_state_dict[k] = checkpoint[k]
    ...
    @torch.inference_mode()
    def forward_extract_features(self, tokens, image=None, modal='image', vqa_answers_tokens=None, vqa_question_tokens=None, cap_answers_tokens=None):
        modal = modal[0] if isinstance(modal, list) else modal
        
     
        vqa_answers_tokens_tmp = vqa_answers_tokens.masked_fill(vqa_answers_tokens == -1, 0)
        vqa_ans_embeddings_tmp = self.tok_embeddings(vqa_answers_tokens_tmp)

      
        vqa_question_tokens_tmp = vqa_question_tokens.masked_fill(vqa_question_tokens == -1, 0)
        vqa_question_embeddings_tmp = self.tok_embeddings(vqa_question_tokens_tmp)

     
        cap_answers_tokens_tmp = cap_answers_tokens.masked_fill(cap_answers_tokens == -1, 0)
        cap_ans_embeddings_tmp = self.tok_embeddings(cap_answers_tokens_tmp)

    
        ignore_mask_vqa_ans = (vqa_answers_tokens == -1).unsqueeze(-1) 
        vqa_ans_embeddings = vqa_ans_embeddings_tmp.masked_fill(ignore_mask_vqa_ans, 0)  

        ignore_mask_vqa_q = (vqa_question_tokens == -1).unsqueeze(-1)  
        vqa_question_embeddings = vqa_question_embeddings_tmp.masked_fill(ignore_mask_vqa_q, 0) 

        ignore_mask_cap_ans = (cap_answers_tokens == -1).unsqueeze(-1) 
        cap_ans_embeddings = cap_ans_embeddings_tmp.masked_fill(ignore_mask_cap_ans, 0)  

        before_connector_feats, after_connector_feats, llm_image_feats = self.extract_features(image, modal)
            
        return before_connector_feats, after_connector_feats, llm_image_feats, vqa_ans_embeddings, vqa_question_embeddings, cap_ans_embeddings
