...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.output_action = ColumnParallelLinear(
            params.dim, 55, bias=False, init_method=default_linear_init,
        )
        ...
        for modal in self.modals:
            ...
            if modal in ['video','depth', 'infra']:
                ...
                if modal == "video":
                    pretrained_path = "./modality_specific_encoders/xrf55_encoders/random_pretrained_encoders/rgb_network.pth"
                elif modal == "depth":
                    pretrained_path = "./modality_specific_encoders/xrf55_encoders/random_pretrained_encoders/depth_network.pth"
                else:
                ...
                self.conv1[modal] = nn.Conv2d(9, clip_width, kernel_size=(3, 3), stride=(2, 2))
                
                # resnet18 = models.resnet18(pretrained=True)
                # self.modality_encoders[modal] = nn.Sequential(*list(resnet18.children())[:-2])
                self.modality_encoders[modal] = WiFi_Encoder()
                
                pretrained_path = "./modality_specific_encoders/xrf55_encoders/random_pretrained_encoders/wifi_network.pth"
                checkpoint = torch.load(pretrained_path)
                encoder_state_dict = self.modality_encoders[modal].state_dict()
                for k, v in encoder_state_dict.items():
