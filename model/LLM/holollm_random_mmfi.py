...
class Transformer(nn.Module):
        ...
        for modal in self.modals:
            ...
            if modal in ['video','depth', 'infra']:
                ...
                if modal == "video":
                    ...
                    pretrained_path = "./modality_specific_encoders/mmfi_encoders/random_pretrained_encoders/rgb_network.pth"
                elif modal == "depth":
                    # self.convnet_conv_depth = nn.Conv1d(in_channels=256, out_channels=64, kernel_size=1, stride=1)
                    pretrained_path = "./modality_specific_encoders/mmfi_encoders/random_pretrained_encoders/depth_network.pth"
                else:
                    pretrained_path = "./modality_specific_encoders/xrf55_encoders/random_pretrained_encoders/infra_network.pth"
                ...
                pretrained_path = "./modality_specific_encoders/mmfi_encoders/random_pretrained_encoders/lidar_network.pth"
                ...
                pretrained_path = "./modality_specific_encoders/mmfi_encoders/random_pretrained_encoders/mmwave_network.pth"
                ...
                pretrained_path = "./modality_specific_encoders/mmfi_encoders/random_pretrained_encoders/wifi_network.pth"
                checkpoint = torch.load(pretrained_path)
                encoder_state_dict = self.modality_encoders[modal].state_dict()
                for k, v in encoder_state_dict.items():
                ...
                pretrained_path = "./modality_specific_encoders/xrf55_encoders/random_pretrained_encoders/rfid_network.pth"
                checkpoint = torch.load(pretrained_path)
                encoder_state_dict = self.modality_encoders[modal].state_dict()
                for k, v in encoder_state_dict.items():
                    encoder_state_dict[k] = checkpoint[k]
    ...
    @torch.inference_mode()
    def forward_extract_features(self, tokens, image=None, modal='image', vqa_answers_tokens=None):
        modal = modal[0] if isinstance(modal, list) else modal
        ...
        ignore_mask = (vqa_answers_tokens == -1).unsqueeze(-1)  
        vqa_ans_embeddings = vqa_ans_embeddings_tmp.masked_fill(ignore_mask, 0)  
        
        before_connector_feats, after_connector_feats, llm_image_feats = self.extract_features(image, modal)
            
        return before_connector_feats, after_connector_feats, llm_image_feats, vqa_ans_embeddings
