...
class Projector(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #     x += self.pos_emb  # the length in our setting is not fixed to 256 so do not use pos_emb
...
class ConvProjector(Projector):
    def _forward(self, x):
        ...
        if not self.training:
            x = rearrange(x_padded.half(), "b (h w) d -> b d h w", h=hw, w=hw)
            x = x.clone()
        else:
            x = rearrange(x_padded.bfloat16(), "b (h w) d -> b d h w", h=hw, w=hw)
