...
class MetaHoloModel(nn.Module):
    ...
    def __init__(self, llama_type, llama_config, llama_ckpt_dir=None, tokenizer_path=None):
        ...
        model = Transformer(model_args)  # the clip is loaded in Transformer by open_clip
        ...
        for cur_pos in range(start_pos, total_len):
            ...
            if temperature > 0:
                ...
                try:
                    ...
                    # next_token = self.sample_top_p(probs, top_p)
                    shape_dim_1 = input_text_mask.shape[0]
                    next_token = torch.zeros((shape_dim_1,1)).cuda()
            else:
    ...
    def extract_features(
        ...
        vqa_answers_tokens_tmp = [self.tokenizer.encode(
        x, bos=True, eos=False) for x in vqa_answers]

        vqa_question_tokens_tmp = [self.tokenizer.encode(
        x, bos=True, eos=False) for x in vqa_questions]

        cap_answers_tokens_tmp = [self.tokenizer.encode(
        x, bos=True, eos=False) for x in cap_answers]

        min_prompt_size = min([len(t) for t in prompt_tokens])
        max_prompt_size = max([len(t) for t in prompt_tokens])
        total_len = min(params.max_seq_len, max_prompt_size)

        # min_vqa_ans_size = min([len(t) for t in answers_tokens])
        max_vqa_ans_size = max([len(t) for t in vqa_answers_tokens_tmp])
        vqa_ans_total_len = max_vqa_ans_size

        max_vqa_question_size = max([len(t) for t in vqa_question_tokens_tmp])
        vqa_question_total_len = max_vqa_question_size

        max_cap_ans_size = max([len(t) for t in cap_answers_tokens_tmp])
        cap_ans_total_len = max_cap_ans_size

       
        tokens = torch.full(
            (bsz, total_len), self.tokenizer.pad_id).cuda().long()
        
        vqa_answers_tokens = torch.full(
            (bsz, vqa_ans_total_len), self.tokenizer.pad_id).cuda().long()

        vqa_question_tokens = torch.full(
            (bsz, vqa_question_total_len), self.tokenizer.pad_id).cuda().long()

        cap_answers_tokens = torch.full(
            (bsz, cap_ans_total_len), self.tokenizer.pad_id).cuda().long()
        
        
        for k, t in enumerate(prompt_tokens):
            tokens[k, : len(t)] = torch.tensor(t).long() 

        for k, t in enumerate(vqa_answers_tokens_tmp):
            vqa_answers_tokens[k, : len(t)] = torch.tensor(t).long()

        for k, t in enumerate(vqa_question_tokens_tmp):
            vqa_question_tokens[k, : len(t)] = torch.tensor(t).long()

        for k, t in enumerate(cap_answers_tokens_tmp):
            cap_answers_tokens[k, : len(t)] = torch.tensor(t).long()
        ...
        before_connector_feats, after_connector_feats, llm_image_feats, vqa_ans_feats, vqa_question_feats, cap_ans_feats = self.llma.forward_extract_features(tokens[:, prev_pos:cur_pos], images, modal=modal, vqa_answers_tokens=vqa_answers_tokens, vqa_question_tokens=vqa_question_tokens, cap_answers_tokens=cap_answers_tokens)
        
        return before_connector_feats, after_connector_feats, llm_image_feats, vqa_ans_feats, vqa_question_feats, cap_ans_feats
