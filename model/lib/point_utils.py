...
class MMFiPointPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # coordinates
        # p = x.contiguous()  # mmfi only has x, y, z
        # B, N, _ = p.shape[:3]
        # idx = self.sample_fn(p, int(N * self.sample_ratio)).long()
        # idx = self.sample_fn(p, self.sample_number).long()
        # idx = self.sample_fn(p, self.sample_number)
        # center_p = torch.gather(p, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
        # query neighbors.
        # _, fj = self.grouper(center_p, p, x.permute(0, 2, 1).contiguous()) # [B, N, 3] -> [B, 3, N] -> [B, 3, 1024, 32]

        # instead of use sample fn, use randomly select first
        point_num = x.shape[1]
        if self.training:
            sample_indices = torch.randperm(point_num)[:self.sample_number]
        else:  
            sample_indices = torch.arange(point_num)[:self.sample_number]
        x = x[:, sample_indices, :].permute(0,2,1).unsqueeze(dim=-1)  
        # [B, 3, 1024] -> [B, channels, 1024, 1]
        fj = self.conv1(x).max(dim=-1, keepdim=True)[0]
