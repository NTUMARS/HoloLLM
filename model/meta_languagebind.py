...
class MetaLanguagebindModel(nn.Module):
    ...
    def extract_features(
        ...
        answers_tokens = [self.tokenizer.encode(
        x, bos=False, eos=False) for x in vqa_answers]
        ...
        max_vqa_ans_size = max([len(t) for t in answers_tokens])
        ...
        for k, t in enumerate(answers_tokens):
        ...
        if len(images.shape) != 5:  # video [32, 15, 3, 224, 224]
            images = images.unsqueeze(dim=2)  # [32, 15, 1, 224, 224]
        images = images.half()  # changed to half
        before_connector_feats, after_connector_feats, llm_image_feats, h_caption = self.llma.forward_extract_features(tokens[:, prev_pos:cur_pos], images, modal=modal, vqa_answers_tokens=vqa_answers_tokens)
