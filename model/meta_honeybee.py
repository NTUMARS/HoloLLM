...
class MetaHoneybeeModel(nn.Module):

    def __init__(self, llama_type, llama_config, llama_ckpt_dir=None, tokenizer_path=None):
        super().__init__()
        
      
        self.ckpt_path = "/home/LLM_ckpt/honeybee/7B-C-Abs-M256/last/"
        honey_config_json = self.ckpt_path + "config.json"

        with open(honey_config_json, 'r') as file:
            honey_config_dict = json.load(file)

        honey_config = HoneybeeConfig.from_dict(honey_config_dict)  # Honeybee config provide this initialize func
        ...
        model = Transformer(honey_config, model_args)  # the clip is loaded in Transformer by open_clip
