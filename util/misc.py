    ...
    def __call__(self, loss, optimizer, model, clip_grad=None, parameters=None, create_graph=False, update_grad=True):
        if update_grad:  
            ...
            if clip_grad is not None:
                ...
                # if args.clip_grad=-1 always obtain this error.
                # what this mean?
                # in OneLLM, clip_grad is set to 2, just copy them
...
# only save projector
def save_projector(output_dir, args, epoch, iteration, model, optimizer, loss_scaler, dataset_state):
    save_dir = os.path.join(output_dir, f"epoch_{epoch}_iter_{iteration:09d}_projector")
    os.makedirs(save_dir, exist_ok=True)  
    ...
    with FSDP.state_dict_type(model, StateDictType.SHARDED_STATE_DICT):
        ...
        # Filter state_dict to keep only trainable parameters
        trainable_state_dict = {name: param for name, param in model.state_dict().items() if "_fsdp_wrapped_module." + name in trainable_params}
    ...
    if args.save_consolidated:
        ...
        with FSDP.state_dict_type(
            ...
            # }
            consolidated_model_state_dict = {
                k: v.to(save_dtype) for k, v in trainable_state_dict.items()
            }
...
def load_projector(args, model, optimizer, loss_scaler):
    start_iter = 0
    start_epoch = 0
    if args.auto_resume:
        ckpt_dirs = glob.glob(os.path.join(args.output_dir, "iter_*")) + glob.glob(os.path.join(args.output_dir, "epoch_*"))
    ...
    if args.resume:
        ...
        with FSDP.state_dict_type(model, StateDictType.SHARDED_STATE_DICT):
            ...
            filtered_state_dict = {k: v for k, v in checkpoint['model'].items() if k in model_dict}
        ...
        loss_scaler.load_state_dict(checkpoint['scaler'])
        # start_iter = int(checkpoint['iter']) + 1
        # change from 9999 to 481, may change later
        start_iter = 0
        if 'epoch' in checkpoint:
