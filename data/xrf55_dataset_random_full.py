...
cap_path = "./datasets/textual_annotations/xrf55/xrf55cap/xrf55_train_random_full.json"
vqa_base_path = "./datasets/textual_annotations/xrf55/xrf55vqa/train_random_full/"

DATASETS = dict(

    xrf55_video=[
        dict(path=cap_path, type='xrf55_video'),
        dict(path=vqa_base_path+"xrfvqa_xrf55_video.json", type='xrf55_video'),
        # xrf55 video here
    ],

    xrf55_depth=[
        dict(path=cap_path, type='xrf55_depth'),
        dict(path=vqa_base_path+"xrfvqa_xrf55_depth.json", type='xrf55_depth'),
        # xrf55 video here
    ],

    xrf55_infra=[
        dict(path=cap_path, type='xrf55_infra'),
        dict(path=vqa_base_path+"xrfvqa_xrf55_infra.json", type='xrf55_infra'),
        # xrf55 video here
    ],

    xrf55_wifi=[
        dict(path=cap_path, type='xrf55_wifi'),
        dict(path=vqa_base_path+"xrfvqa_xrf55_wifi.json", type='xrf55_wifi'),
        # xrf55 video here
    ],

    xrf55_rfid=[
        dict(path=cap_path, type='xrf55_rfid'),
        dict(path=vqa_base_path+"xrfvqa_xrf55_rfid.json", type='xrf55_rfid'),
        # xrf55 video here
    ]
...
class OneStageSenseDataset(Dataset):
    ...
    def __getitem__(self, index, expect_type=None):
        ...
        # here change data_type? or change in the model, just change in the model
        if image is None:
